...
1. Open this (https://github.com/mcp/microsoft/playwright-mcp) and choose Install MCP Server or hit cmd + shift + P to open search and type “MCP”.
...
# TOPIC: Understanding Engineering drawings as a whole

Engineering, specifically manufacturing, fabrication, assemble are a well documented process. Each step of the process is designed to be precise and cost effective. To achieve this, Engineering drawings are used.

Engineering drawings are sort of a mode of communication in the language of design. It represents a precise, technical and standardized link between one engineering to the another, regardless of their cultural context.

## How are engineering drawings used?

The key to designing a system like this is to focus broadly on the entire use case of the system that it's based on. Which is engineering drawings. If we hyper focus on a specific use case or intent, I believe, It would become difficult to adapt an existing system to various use cases that might pop-up.

There might be more use cases that are specific to the requirements of the company but if we focus on all of these, adapting to the new use case will be easier and more seamless as it might most likely be based on the aforementioned uses.

## Who uses engineering drawings?

Understanding the user base is important. Based on the distribution of variation of the users we can create a UX that can work for everyone.

1. Machinist:

   - These sets of individuals might be more technical and they might look for more details rather than how the details are presented. They are used to skimming through raw engineering drawing with no need of an UX layer on top of it.
2. Assembly Techs:

   - These individuals would definitely need an UX. As they specialize in putting together parts, they would benefit not speeding time reading through a huge sheet.
3. QC:

   - UX would make their job the easiest. They work with very few data points. What they would be doing a lot is cross-referencing drawings to the actual parts.
4. Procurement:

   - This team would not care about how it's put together or how it's manufactured but they do care about the quantity.
5. Project Managers:

   - Tracking the progress of the project, hence they do not need highly technical information such as tolerance and dimensions.

# Defining the Problem

It is pretty clear that the current workflow is pretty exhausting. The problem becomes obvious when you look at the day-to-day flow.

## Understanding the problem at each step

Going back to the user flow, each of the steps or phases have a sort of bottleneck.

1. ### Planning and Sourcing:

   1. Project Managers are translating designs into tasks, manually routing information rather than having a connected system. A pathway even.
   2. The procurement team needs to perform data entry work. Visually finding the BOM, and manually type part numbers and quantities into ERP systems like SAP or Oracle.
   3. There can be a problem where human error can easily happen, risking the accuracy.
2. ### Making Phase:

   1. Machinists are required to get physical papers or digital copies.
   2. An automated system that serves and sorts tools based on digitally analysed drawings are absent.
   3. Not having instant access to proper torque figures and assembly procedures increases the assembly time reducing efficiency.
3. ### Quality Control:

   1. They are made to manually open drawings to find tolerances and specifications.
   2. They must use tools to measure the part and then type out the reports.
   3. Reports are fragmented into different systems.
